(function () {
  ...
  context.fillStyle = 'rgba(0,195,255,0.8)';
  ...
  function createParticles() {
    ...
    for (let index = 0; index < 399; index += 1) {
      particles.push({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, xa: Math.random() * 2 - 1, ya: Math.random() * 2 - 1, max: 6000 });
    }
  }
  ...
  function draw() {
    ...
    particles.forEach((particle, particleIndex) => {
      ...
      for (let targetIndex = particleIndex + 1; targetIndex < particles.length; targetIndex += 1) connect(particle, particles[targetIndex]);
    });
